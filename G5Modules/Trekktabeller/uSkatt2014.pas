...
const
  KLASSE1_VANLIG               = 48800;
  KLASSE2_VANLIG               = 72000;
  KLASSE1_FINNMARK             = 63800;
  KLASSE2_FINNMARK             = 87000;

var
  TOPPSKATT_PROSENT            : Tabell3 = ( 0.0, 0.0, 12.0 );
const
  TOPPSK_TRINN1                = 9.0;
  TOPPSK_TRINN1_FINNMARK       = 7.0;

  TOPPSKATT_LAV                : Tabell3 = ( 0, 527400, 527400 );
  TOPPSKATT_HOY                : Tabell3 = ( 0, 857300, 857300 );
  Skattore                     : Extended = 14.05;


var
  FELLES_SKATT_PROSENT:Currency = 0.00;
const
  FELLES_SKATT_VANLIG          = 12.95;
  FELLES_SKATT_FINNMARK        = 9.45;


  LAV_GRENSE_TRYGDEAVGIFT      = 49749;
  HOY_GRENSE_TRYGDEAVGIFT      = 58929;
  AVG_FRI_TRYGDEAVGIFT         = 39600;
  ...
  ANV_MINSTE_FRAD_PROSENT      = 37.84;
  ANV_MINSTE_FRAD_PROSENT_PENSJ= 23.76;
  MIN_ANV_MINSTE_FRADRAG       = 3520;
  MAX_ANV_MINSTE_FRADRAG       = 74052;
  MAX_ANV_MINSTE_FRADRAG_PENSJ = 61952;

  MINSTE_FRAD_PROSENT          = 43.0;

  MIN_MINSTE_FRADRAG           = 4000;
  MAX_MINSTE_FRADRAG           = 84150;
...
Function TSkatt2014.GetSumSkatt : LongInt;
Var
  OrgTrgrl : Extended;
  OrgPer   : SmallInt;
begin
...
Function TSkatt2014.GetSumSkattNetto : LongInt;
Var
  OrgTrgrl : Extended;
  OrgPer   : SmallInt;
  Tillegg  : Extended;
...
begin
  ...
  begin
    TOPPSKATT_PROSENT[1]:=TOPPSK_TRINN1_FINNMARK;
  ...
  begin
    TOPPSKATT_PROSENT[1]:=TOPPSK_TRINN1;
...
Procedure TSkatt2014.CalcSkatt;
begin
...
Procedure TSkatt2014.CalcOppgrossing;           { ref id 986 }
begin
...
Procedure TSkatt2014.SettVanlig;
begin
...
Procedure TSkatt2014.SettVanligNetto;
begin
...
Procedure TSkatt2014.SettPensjonist;
begin
...
Procedure TSkatt2014.SettStfradrag;
begin
...
Procedure TSkatt2014.SettSjomann;
begin
...
Procedure TSkatt2014.BeregnAarsGrunnlag;
begin
...
Procedure TSkatt2014.BeregnOverskTrekk;
var
...
begin
  ...
  Case Tabelltype of
    ...
    TABTYPE_VANLIG : Overskytende_Prosent := 0.54;
  ...
  Case Ws_Tabellnr of
    ...
    7300, 7400 : Overskytende_Prosent := 0.54;

    7350, 7450 : Overskytende_Prosent := 0.47;

    7500, 7600 : Overskytende_Prosent := 0.39;

    7550, 7650 : Overskytende_Prosent := 0.45;
    ...
    6300, 6400 : Overskytende_Prosent := 0.50;

    6350, 6450 : Overskytende_Prosent := 0.44;

    6500, 6600 : Overskytende_Prosent := 0.36;

    6550, 6650 : Overskytende_Prosent := 0.41;

    6700, 6800 : Overskytende_Prosent := 0.41;

    7150, 7250 : Overskytende_Prosent := 0.47;

    7160, 7260 : Overskytende_Prosent := 0.39;

    7170, 7270 : Overskytende_Prosent := 0.45;
...
Procedure TSkatt2014.BeregnFradrag;
var
...
Procedure TSkatt2014.BeregnNettoskatt;
begin
...
Procedure TSkatt2014.BeregnTrygdeavgift;
begin
...
Procedure TSkatt2014.BeregnToppskatt;
var
  Topp_Skatt_Trinn1     : Extended; // 2000
  Topp_Skatt_Trinn2     : Extended; // 2000
begin

  Topp_Skatt_Trinn1 := 0.0;
  Topp_Skatt_Trinn2 := 0.0;

  if ( Aars_Inntekt > TOPPSKATT_LAV[Klasse] ) then
  begin

    if ( Aars_Inntekt < TOPPSKATT_HOY[Klasse] ) then
       Topp_Skatt_Trinn1 := ( Aars_Inntekt - TOPPSKATT_LAV[Klasse] )*TOPPSKATT_PROSENT[1] / 100.0
    else
       Topp_Skatt_Trinn1 := ( TOPPSKATT_HOY[Klasse] - TOPPSKATT_LAV[Klasse] )*TOPPSKATT_PROSENT[1] / 100.0;

  end;  { slutt if †rsinntekt > lav toppskatt }

  if ( Aars_Inntekt > TOPPSKATT_HOY[Klasse] ) then
    Topp_Skatt_Trinn2 := ( Aars_Inntekt - TOPPSKATT_HOY[Klasse] )*TOPPSKATT_PROSENT[2] / 100.0;

  Sum_Topp_Skatt := Topp_Skatt_Trinn1 + Topp_Skatt_Trinn2;

end;  { slutt BeregnToppskatt }

Procedure TSkatt2014.BeregnTrekk;
var
  Trekk     : LongInt;
begin

  Trekk := 0;
  Sum_Trekk := Kommune_Skatt + Felles_Skatt + Trygde_Avgift + Sum_Topp_Skatt;
  ...
  if ( Trekk > Ws_TrGrl_Avrundet ) then       { Ny i 1996 }
    Trekk := l_runde(Ws_TrGrl_Avrundet);

  Trekk := Trekk + l2_runde( Overskytende_Trekk ); { Ny i 1996 }
  Parm_Ut_Trekk := Trekk;

end;  { slutt BeregnTrekk }

Procedure TSkatt2014.BeregnAarsTrekk;     { Ref id 986 }
begin
